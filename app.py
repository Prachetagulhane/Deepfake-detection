...
def upload_video():
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                while cap.isOpened():
                    ...
                    print(f"Frame {frame_count} - Input Tensor Mean: {input_tensor.mean().item()}, Std: {input_tensor.std().item()}")

                    # Predict
                    with torch.no_grad():
                        output = model(input_tensor)
                        print(f"Frame {frame_count} - Raw Model Output: {output}")
                        prob = torch.sigmoid(output).item()
                        prediction = "FAKE" if prob > 0.5 else "REAL"

                    # Save frame as image
                    frame_image_name = f'frame_{frame_count}.jpg'
                    frame_image_path = os.path.join(app.config['UPLOAD_FOLDER'], frame_image_name)
                    ...
                    frame_results.append({
                        ...
                        "confidence": round(prob, 2),
                        "image_path": f'uploads/{frame_image_name}'  # Path for static serving
                    })
                ...
                return render_template(
                    ...
                    video_url=f'uploads/{filename}'  # Static path for video
                )
            except Exception as e:
    ...
    return render_template('upload_video.html', results=[])
